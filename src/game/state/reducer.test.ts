...
describe('step', () => {
  it('runs a registered command and reports it', () => {
    const result = step(createInitialGameState(), executeCommand('status', []), registry);
  ...
  it('counts only commands that actually executed', () => {
    const executed = step(createInitialGameState(), executeCommand('status', []), registry);
  ...
  it('rejects an action naming an unregistered command', () => {
    const result = step(createInitialGameState(), executeCommand('nmap', []), registry);
  ...
  it('never mutates the state it is given', () => {
    ...
    step(state, executeCommand('status', []), registry);
  ...
  it('is deterministic for identical input', () => {
    const run = () => {
      ...
      for (const command of ['status', 'help', 'status']) {
        state = step(state, executeCommand(command, []), registry).state;
      }
      return state;
    };
  ...
  describe('gates', () => {
    it('blocks a mission-only command with no active mission', () => {
      ...
      const result = step(createInitialGameState(), executeCommand('probe', []), local);
      expect(result.outputs[0]?.text).toBe('No active mission. Select a mission first.');
      expect(result.events).toContainEqual({
        type: 'COMMAND_REJECTED',
        commandId: 'probe',
    ...
    it('blocks a command demanding more access than the mission has', () => {
      ...
      const result = step(withMission(createInitialGameState()), executeCommand('probe', []), local);
  ...
  describe('detection', () => {
    ...
    it('does not fail an already-failed mission a second time', () => {
      const local = createCommandRegistry([...COMMAND_SPECS, probe({ detectionCost: 100 })]);
      const first = step(withMission(createInitialGameState()), executeCommand('probe', []), local);
      const second = step(first.state, executeCommand('probe', []), local);
